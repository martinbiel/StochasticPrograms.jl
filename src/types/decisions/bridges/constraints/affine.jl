...
function MOI.modify(model::MOI.ModelLike, bridge::AffineDecisionConstraintBridge{T,S}, change::DecisionStateChange) where {T,S}
    f = bridge.decision_function
...
function MOI.modify(model::MOI.ModelLike, bridge::AffineDecisionConstraintBridge{T}, ::DecisionsStateChange) where T
    f = bridge.decision_function
    unbridged_func = zero(AffineDecisionFunction{T})
    variables_to_remove = Vector{Int}()
    decisions_to_remove = Vector{Int}()
    # First, unbridge the constraint function and cache

...
Functor object for the trust-region L-shaped algorithm. Create by supplying a `TR` object through `regularize ` in the `LShapedSolver` factory function and then pass to a `StochasticPrograms.jl` model.
